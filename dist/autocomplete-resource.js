...
angular.module('autocomplete-resource',['ui.bootstrap'])
    .directive('autocompleteResource', function ($timeout) {
        return {
            ...
            template: '<div class="form-group"> <label ng-show="label">{{label}}</label> <div class="input-group" ng-class="inputGroupSize"> <span class="input-group-addon"><img src="bower_components/autocomplete-resource/dist/images/ajax-loader-autocomplete.gif" style="height:15px;" ng-show="searching" /><span ng-show="!searching" class="glyphicon glyphicon-search"></span></span> <input ng-readonly="readOnly" ng-disabled="ngdisabled" tooltip-placement="{{tooltipplacement}}" tooltip="{{getTooltip()}}" id="inputFilter" ng-required="isrequired" autocomplete="off" class="{{classautocomplete}} form-control" id="{{idautocomplete}}" type="text" ng-model="modelfilter" ng-trim="false" placeholder="{{placeholder}}" ng-keydown="keyDown($event)"  ng-blur="handleBlur()" /> <span ng-if="!model && showArrowBtn" class="input-group-btn"> <button ng-disabled="ngdisabled || readOnly" ng-click="searchAll()" ng-blur="handleBlur()" class="btn btn-default" type="button"><span class="glyphicon glyphicon-chevron-down"></span></button> </span> <span ng-if="model && !clearInputOnSelectionParsed" class="input-group-btn"> <button ng-disabled="ngdisabled || readOnly" ng-click="removeItem(!clearOnBlurParsed)" class="btn btn-default" type="button"><span class="glyphicon glyphicon-remove"></span></button> </span> </div> <div style="position:relative;" ng-class="{\'top-label\':haveLabel(),\'top-sin-label\':!haveLabel(),\'tieneAlert\': tieneAlert() }"  > <alert style="position:absolute;top:0px;left:0px;" type="danger" ng-if="(model==null && isrequired && tieneAlert())" ><span class="glyphicon glyphicon-warning-sign"></span> {{requiredmsj}}</alert> <ul class="autocomplete dropdown-menu" style="display:block;position:absolute;top:0px;left:0px;" ng-hide="!listOpened || selected"  role="menu"  > <li ng-hide="items.length>0" class="autocomplete-warning-label"><span class="glyphicon glyphicon-info-sign"></span> No se encontraron resultados </li> <li class="item" ng-class="{\'ellipsis\':withEllipsis(),\'active\':isCurrent($index)}" ng-repeat="item in items track by $index" ng-mousedown="handleSelection(item)" style="cursor:pointer"  ng-mouseenter="setCurrent($index)" popover-template="{{getPopoverTemplate()}}" popover-trigger="mouseenter" popover-placement="{{getPopoverPlacement()}}" > <span ng-if="!imgItemPreviewSrcAttribParsed" ng-if="itemicon" class="itemicon" ng-class="itemicon" ></span> <span ng-if="imgItemPreviewSrcAttribParsed"><img class="autocomplete-img-preview"  ng-src="{{getItemLabel(item,imgItemPreviewSrcAttribParsed)}}" /></span> <span  class="itemlabel">{{getItemLabel(item,itemlabel)}}</span> <span ng-if="itemdescrip" class="itemdescrip">{{getItemLabel(item,itemdescrip)}}</span> <span ng-if="itemdescrip2" class="itemdescrip2">{{getItemLabel(item,itemdescrip2)}}</span> <span class="autocomplete-reset-float"></span> </li> <li ng-if="withItemDetail()" ng-if="items[current]" class="autocomplete-lupa"> <div ng-if="itemlabel" class="itemlabel" >{{getItemLabel(items[current],itemlabel)}}</div> <div ng-if="itemdescrip" class="itemdescrip" >{{getItemLabel(items[current],itemdescrip)}}</div> <div ng-if="itemdescrip2" class="itemdescrip2" >{{getItemLabel(items[current],itemdescrip2)}}</div> </li> </ul> </div> </div> <script type="text/ng-template" id="autocompleteResurceTemplate.html"> <div class="autocomplete-popover"> <div class="itemlabel">{{getItemLabel(item,itemlabel)}}</div> <div class="itemdescrip" >{{getItemLabel(item,itemdescrip)}}</div> <div class="itemdescrip2">{{getItemLabel(item,itemdescrip2)}}</div> </div> </script>'
        };
    });
